...
ap.add_argument("--models_out", default="voting.pickle")
args = ap.parse_args()
...
for train_idx, valid_idx in cv.split(X):
    ...
    rfc = RandomForestClassifier(n_estimators=3000, random_state=123)
    gbc = GradientBoostingClassifier(random_state=123)

    voting = VotingClassifier(estimators=[('svm', svm), ('rfc', rfc), ('gbc', gbc)], voting='soft',  weights=[5,3,4])
